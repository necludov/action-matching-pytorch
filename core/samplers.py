...
class HMC:
    def __init__(self, target, T_max=None, step_size=None, alpha=1.0, jitter=True):
    ...
    def adapt_eps(self, x_0, n):
        batch_size = x_0.shape[0]
#         self.eps = self.find_reasonable_eps(x_0)
        self.eps = self.T_max/20.0
...
#             acceptance_prob = torch.exp(log_accept_p.mean())
            running_ar = (1.-1./(i+time_0))*running_ar + 1./(i+time_0)*(0.8-acceptance_prob)
...
#             running_eps = i**(-kappa)*torch.exp(log_eps)+(1.-i**(-kappa))*running_eps
            ...
            grad_estimate *= t/self.T_max*((x_next - x_next_mean).norm(2,dim=1,keepdim=True)-(x - x_mean).norm(2,dim=1,keepdim=True))
...
class RWMH:
    def __init__(self, target, sigma=1e-2):
        self.device = target.device
        self.target = target
        self.sigma = sigma
