...
#             m_idx += 1
#             yemb = modules[m_idx](self.act(yemb))
#             m_idx += 1
#             temb = temb + yemb
...
class ActionNet(nn.Module):
    def __init__(self, config):
        ...
        # condition on time
        modules = []
        modules.append(nn.Linear(1, nf*4))
        modules[-1].weight.data = default_initializer()(modules[-1].weight.data.shape)
        nn.init.zeros_(modules[-1].bias)
        modules.append(nn.Linear(nf*4, nf*4))
        modules[-1].weight.data = default_initializer()(modules[-1].weight.data.shape)
    ...
    def forward(self, t, x, y=None):
        ...
        modules = self.all_modules
        m_idx = 0
        temb = modules[m_idx](t.view(-1,1))
        m_idx += 1
        temb = modules[m_idx](torch.tanh(temb))
        m_idx += 1
        temb = modules[m_idx](torch.tanh(temb))
        m_idx += 1
        if self.conditional:
            yemb = modules[m_idx](y)
            m_idx += 1
            yemb = modules[m_idx](torch.tanh(yemb))
            m_idx += 1
            yemb = modules[m_idx](torch.tanh(yemb))
            m_idx += 1
